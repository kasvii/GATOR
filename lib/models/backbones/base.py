...
def prepare_network(args, load_dir='', is_train=True):
    ...
    if is_train or load_dir:
        ...
        if cfg.MODEL.name == 'pose2mesh_net':
            ...
            model = models.Graphormer.get_model(num_joint=main_dataset.joint_num, embed_dim=256, depth=4, graph_adj=main_dataset.graph_Adj, GCN_depth=2, J_regressor=J_regressor) # 
    ...
    def __init__(self, args, load_dir):
        ...
        if cfg.TRAIN.wandb:
            wandb.init(config=cfg,
                   ...
                   name='GSAT_pretrain/' + cfg.MODEL.name,
                   dir=cfg.output_dir,
                   job_type="training",
                   reinit=True)
